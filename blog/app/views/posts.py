...
def send_posts():
    ...
    if not current_user.is_authenticated:
        ...
        if form.state.data == '0':
            ...
            p = True
        info = Posts(title=form.title.data, article=form.article.data, user=current_user, state=p)
        # print(info.get_id())
        info.save()
...
def posts_detail(id):
    ...
    return render_template('posts/page_detail.html', p=p, a=a, form=form, data=data, data_all=data_all)
...
def search():
    ...
    pagination = Posts.query.filter(or_(Posts.title.contains(search_info), Posts.article.contains(search_info)),
                                    Posts.pid == 0, Posts.state == True).order_by \
